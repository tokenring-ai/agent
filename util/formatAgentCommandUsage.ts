...
export function formatAgentCommandUsage<Schema extends AgentCommandInputSchema>(
  ...
  if (remainder) {
    usageParts.push(
      remainder.required ? `<${remainder.name}...>` : `[${remainder.name}...]`,
    );
...
export function formatAgentCommandUsageError<
  Schema extends AgentCommandInputSchema,
>(command: TokenRingAgentCommand<Schema>, message: string) {
