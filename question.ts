...
export function isTreeBranch(
  node: TreeLeaf,
): node is Extract<TreeLeaf, { children: Array<TreeLeaf> }> {
  return "children" in node;
}

export function isTreeValueLeaf(
  node: TreeLeaf,
): node is Extract<TreeLeaf, { value: string }> {
...
export type ParsedTreeSelectQuestion = z.output<
  typeof TreeSelectQuestionSchema
>;
...
export type ParsedFileSelectQuestion = z.output<
  typeof FileSelectQuestionSchema
>;
...
export function getDefaultQuestionValue<
  T extends | ParsedTextQuestion
    | ParsedFormQuestion
    | ParsedFileSelectQuestion
    | ParsedTreeSelectQuestion,
>(field: T): ResultTypeForQuestion<T> {
  switch (field.type) {
    case "form": {
