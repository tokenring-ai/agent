...
describe("Agent Integration Tests", () => {
  ...
  describe("Agent with Command Service Integration", () => {
    it("should register commands through command service", async () => {
      ...
      const testCommand = {
        ...
        execute: mock().mockResolvedValue("Command executed"),
    ...
    it("should handle chat messages through default command", async () => {
      ...
      const chatCommand = {
        ...
        execute: mock().mockResolvedValue("Chat handled"),
        help: "Chat help",
      };
    ...
    it("should handle unknown commands gracefully", async () => {
      ...
      expect(commandService.executeAgentCommand(agent, "/unknown command")).rejects.toThrow();
    });
  });
  ...
  describe("Agent with Lifecycle Service Integration", () => {
    ...
    beforeEach(() => {
      testHook = {
        ...
        callbacks: [new HookCallback(BeforeAgentInput, mock()), new HookCallback(AfterAgentInputSuccess, mock())],
      };
    ...
    it("should execute hooks during agent lifecycle", async () => {
      ...
      expect(callback?.callback).toHaveBeenCalledWith(expect.any(BeforeAgentInput), agent);
    });
    ...
    it("should handle hook execution in agent context", async () => {
      ...
      expect(callback?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), agent);
    });
  ...
  describe("Manager Integration", () => {
    it("should create agents through manager", async () => {
      ...
      const managedAgent = manager.spawnAgent({ agentType: "integration-test", headless: true });
    ...
    it("should handle sub-agent creation", async () => {
      ...
      const subAgent = manager.spawnSubAgent(agent, "integration-test", {
        headless: true,
      });
  ...
  describe("Concurrent Operations", () => {
    it("should handle multiple input events", () => {
      ...
      expect(historyState.commands).toEqual(["message 1", "message 2", "message 3"]);
    });
  });
  ...
  describe("Error Recovery Integration", () => {
    it("should recover from command execution errors", async () => {
      const failingCommand = {
        ...
        execute: mock().mockRejectedValue(new Error("Command failed")),
        help: "Failing help",
      };
      ...
      expect(commandService.executeAgentCommand(agent, "/fail")).rejects.toThrow();
      ...
      const workingCommand = {
        ...
        execute: mock().mockResolvedValue("Success"),
        help: "Working help",
      };
      ...
      expect(commandService.executeAgentCommand(agent, "/work")).rejects.toThrow("Cannot get abort signal");
