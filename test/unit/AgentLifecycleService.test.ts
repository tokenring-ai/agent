...
const mockHook: HookSubscription = {
  ...
  callbacks: [new HookCallback(AfterAgentInputSuccess, mock()), new HookCallback(BeforeAgentInput, mock())],
};
...
const anotherHook: HookSubscription = {
  ...
  callbacks: [new HookCallback(AfterAgentInputSuccess, mock())],
};
...
describe("AgentLifecycleService", () => {
  ...
  describe("Hook Registration", () => {
    ...
    it("should add multiple hooks", () => {
      service.addHooks({ ...mockHook, name: "hook1" }, { ...anotherHook, name: "hook2" });
    ...
    it("should get hook entries", () => {
      ...
      expect(entries).toEqual(expect.arrayContaining([expect.arrayContaining(["test-hook"]), expect.arrayContaining(["another-hook"])]));
    });
  });
  ...
  describe("Hook Execution", () => {
    ...
    it("should execute hooks after agent input success", async () => {
      ...
      expect(callback?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
    });
    ...
    it("should execute hooks before agent input", async () => {
      ...
      expect(callback?.callback).toHaveBeenCalledWith(expect.any(BeforeAgentInput), mockAgent);
    });
    ...
    it("should handle hooks with missing callback types", async () => {
      ...
      expect(testCallback?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
      expect(partialCallback?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
    });
    ...
    it("should execute all enabled hooks of the specified type", async () => {
      ...
      const testCallback = mockHook.callbacks.find(c => c.hookConstructor === AfterAgentInputSuccess);
      expect(testCallback?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
      expect(hook1.callbacks[0]?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
      expect(hook2.callbacks[0]?.callback).toHaveBeenCalledWith(expect.any(AfterAgentInputSuccess), mockAgent);
    });
    ...
    it("should handle hook execution errors", async () => {
      const errorHook: HookSubscription = {
        ...
        callbacks: [new HookCallback(AfterAgentInputSuccess, mock().mockRejectedValue(new Error("Hook failed")))],
      };
      ...
      expect(service.executeHooks(new AfterAgentInputSuccess(requestData, responseData), mockAgent)).rejects.toThrow("Hook failed");
    });
  });

  describe("Hook Types", () => {
    it("should handle all hook types", () => {
      const hookTypes = [AfterAgentInputSuccess, BeforeAgentInput];
      ...
      hookTypes.forEach(HookType => {
        const hook: HookSubscription = {
          ...
          callbacks: [new HookCallback(HookType, mock())],
        };
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle non-existent hooks when enabling", () => {
      ...
      expect(() => service.setEnabledHooks(["non-existent"], mockAgent)).toThrow();
    });
  });
});
