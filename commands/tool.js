...
export async function execute(remainder, registry) {
 ...
 if (directOperation) {
  ...
  if (!["enable", "disable", "set"].includes(operation)) {
   chatService.errorLine("Unknown operation. Usage: /tools [enable|disable|set] <tool1> <tool2> ...");
   return;
  }
  ...
  chatService.systemLine("Current enabled tools: " + (registry.tools.getEnabledToolNames().join(" ") || "none"));
  return;
 }
 ...
 const buildToolTree = () => {
  ...
  const sortedPackages = Object.keys(toolsByPackage).sort((a, b) => a.localeCompare(b));
