...
export async function execute(remainder, registry) {
 ...
 if (directOperation) {
  ...
  if (!["enable", "disable", "set"].includes(operation)) {
   chatService.errorLine("Unknown operation. Usage: /resources [enable|disable|set] <resource1> <resource2> ...");
   return;
  }
  ...
  switch (operation) {
   ...
   case "set": {
    ...
    chatService.systemLine(`Set enabled resources: ${resourceNames.join(" ")}`);
    break;
   }
  }

  chatService.systemLine("Current enabled resources: " + (registry.resources.getEnabledResourceNames().join(" ") || "none"));
  return;
 }
 ...
 try {
  ...
  if (selectedResources) {
   ...
   chatService.systemLine(`Set enabled resources: ${selectedResources.join(", ")}`);
  } else {
 ...
 for (const resourceName of resourceNames) {
  ...
  for (let i = 0; i < segments.length; i++) {
   if (i === segments.length - 1) {
    ...
    let child = leaf.find(c => c.name === segments[i] && c.children != null);
    if (!child) {
     child = {
      name: segments[i],
