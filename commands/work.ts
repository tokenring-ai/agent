...
export const description = "/work [message] - Runs the agents work handler with the message";

export async function execute(remainder: string, agent: Agent): Promise<void> {
  if (!remainder?.trim()) {
    agent.infoLine("Please provide a message indicating the work to be completed");
    return;
  }
  ...
  if (agent.options.workHandler) {
    return await agent.options.workHandler(remainder, agent);
  } else {
