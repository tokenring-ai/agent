import ChatService from "../ChatService.js";
  ...
  export function execute(remainder, registry) {
    ...
    const mode = typeof chatService.getMode === "function" ? chatService.getMode() : "(none)";

    chatService.systemLine("Current settings:");
    chatService.systemLine(`Model: ${model}`);
    chatService.systemLine(`Active registry: ${activeServices.length > 0 ? activeServices.join(", ") : "(none)"}`);
    chatService.systemLine(`Active tools: ${activeTools.length > 0 ? activeTools.join(", ") : "(none)"}`);
