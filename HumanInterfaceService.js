...
export default class HumanInterfaceService extends Service {
  ...
  * @type {string}
  */
 description = 'Provides a way to ask the user for a selection from a list of items.';
 ...
 async askForSelection({title, items}) {
  throw new Error('Method "askForSelection()" must be implemented by subclass.');
 }
 ...
 async askForMultipleSelections({title, items, message}) {
  throw new Error('Method "askForMultipleSelections()" must be implemented by subclass.');
 }
 ...
 async askForFileSelection(fileSystem, options = {}) {
  const buildTree = async (path = '') => {
   ...
   try {
    for await (const itemPath of fileSystem.getDirectoryTree(path, {recursive: false})) {
